...
d3.csv("data/library-circulation-by-cardholder-type.csv").then((data) => {
  ...
  types.forEach((t) => {
    d3.select("#type-select").append("option").attr("value", t).text(t);
  });
  ...
  d3.select("#toggle-view").on("click", () => {
    ...
    d3.select("#toggle-view").text(
      viewMode === "cluster" ? "Switch to Stacked Bars" : "Switch to Cluster"
    );
  });
});
  ...
  if (viewMode === "cluster") {
    ...
    const clusterNodes = svg
      ...
      .on("mouseover", (event, d) => {
        ...
        // highlight both years for the same branch
        svg
          .selectAll(".node circle")
          .attr("opacity", (node) =>
            node.BranchCode === d.BranchCode ? 1 : 0.15
          )
          .attr("stroke-width", (node) =>
            node.BranchCode === d.BranchCode ? 3 : 1
          )
          .attr("stroke", (node) =>
            node.BranchCode === d.BranchCode ? "#000" : "#fff"
          );
    ...
    `;
      ...
      .on("click", (event, d) => {
        window.location.href = `branch.html?branch=${d.BranchCode}`;
      });
    clusterNodes
      .append("circle")
    ...
    simulation = d3
      .forceSimulation(clusterBase)
      .force(
        "x",
        d3
          .forceX((d) => (d.Year === "2022" ? width / 4 : (3 * width) / 4))
          .strength(0.3)
      )
      ...
      .force(
        "collision",
        d3.forceCollide().radius((d) => sizeScale(d.total || d.Circulation) + 2)
      )
      .on("tick", () =>
        clusterNodes.attr("transform", (d) => `translate(${d.x},${d.y})`)
      );
    ...
    const totalByYear = {
      2022: d3.sum(
        filtered.filter((d) => d.Year === "2022"),
        (d) => d.Circulation
      ),
      2023: d3.sum(
        filtered.filter((d) => d.Year === "2023"),
        (d) => d.Circulation
      ),
    };
    ...
    svg
      ...
      .text((d) =>
        d === "2022"
          ? `Total Circulation: ${d3.format(",")(totalByYear[d])}`
          : `Total Circulation: ${d3.format(",")(totalByYear[d])} (${pct} %)`
      );
    ...
    const nested = d3.rollups(
      ...
      (v) => ({
        Child: d3.sum(
          v.filter((d) => d.CardholderType === "Child"),
          (d) => d.Circulation
        ),
        Teen: d3.sum(
          v.filter((d) => d.CardholderType === "Teen"),
          (d) => d.Circulation
        ),
        Adult: d3.sum(
          v.filter((d) => d.CardholderType === "Adult"),
          (d) => d.Circulation
        ),
      }),
      (d) => d.Year,
    ...
    svg
      ...
      .enter()
      .append("rect")
      .attr("x", (d) => x0(d.data.BranchCode) + x1(d.data.Year))
      .attr("y", (d) => yScale(d[1]))
...
function highlightBranch(selectedBranch) {
  ...
  const branchNodes = svg
    .selectAll(".node")
    .filter((d) => d.BranchCode === selectedBranch);
  ...
  if (nodesData.length === 2) {
    ...
    svg
      ...
      .text(
        `${pct.toFixed(1)}% ${pct < 0
          ? "Ciculation Decrease Compared to 2022"
          : "Ciculation Increase Compared to 2022"
        }`
      );
  }
}
...
function highlightBranches(branchCodes) {
  ...
  svg
    .selectAll(".node circle")
    .attr("stroke", (d) =>
      branchCodes.includes(d.BranchCode) ? "#000" : "#fff"
    )
    .attr("stroke-width", (d) => (branchCodes.includes(d.BranchCode) ? 3 : 1.2))
    .attr("opacity", (d) => (branchCodes.includes(d.BranchCode) ? 1 : 0.25));
}
